...
module gflist_mod
    ...
    type, extends (ghostlist_t) :: gflist_t
    contains
        procedure :: init01_sublist => init01_gflist
    end type
contains

    ! Both stream and collision are initialized by default.
    subroutine init01_gflist(this,mlist,istinmlist,indinmlist,ist)
        class(gflist_t),target :: this
        class(mainlist_t), pointer, intent(in)  :: mlist
        integer,intent(in),optional             :: ist
        integer,intent(in)                      :: istinmlist,indinmlist
        type(streamghostf_t)                    :: str
        type(blankcollision_t)                  :: col
